...
function normalizeCity(s: string): string {
  return s
    ...
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-z0-9\s]/g, " ")
    .replace(/\b(al|el|the|city|municipality|governorate)\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}

/**
 * Map a detected/API city name onto the closest name in our country city list.
 * Returns exact list value or "" if no reasonable match.
 */
export function matchCityToList(
  ...
  const nDet = normalizeCity(raw);
  if (!nDet) return "";

  // exact normalized
  for (const c of list) {
    if (normalizeCity(c) === nDet) return c;
  }

  // detected contains list city or list city contains detected
  let best: { city: string; score: number } | null = null;
  for (const c of list) {
    const nList = normalizeCity(c);
    if (!nList) continue;
    let score = 0;
    if (nDet === nList) score = 100;
    else if (nDet.includes(nList) || nList.includes(nDet)) score = 80 + Math.min(nList.length, 20);
    else {
      // token overlap
      const t1 = new Set(nDet.split(" ").filter(Boolean));
      const t2 = nList.split(" ").filter(Boolean);
      const hit = t2.filter((t) => t1.has(t)).length;
      if (hit > 0) score = 40 + hit * 15;
    }
    if (score > 0 && (!best || score > best.score)) best = { city: c, score };
  }
